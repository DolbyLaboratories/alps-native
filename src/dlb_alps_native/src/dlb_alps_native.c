...
struct alps_ctx_t
{
    ...
    callback_ctx             presentations_cb_ctx;
};
...
{
    free(presentation->language);
    free(presentation->label);
    ...
    *presentations_changed = 0;
    ...
    if (preselections != NULL)
    {
        ...
        if (preselections_count > ctx->presentations_count)
        {
            ...
            memset(
                ctx->presentations + ctx->presentations_count,
                0,
                sizeof(*ctx->presentations) * (preselections_count - ctx->presentations_count));
        ...
        for (i = 0; i < ctx->presentations_count; i++)
        {
            ...
            if (pres->presentation_id != prsl->preselection_tag)
            {
                *presentations_changed = 1;
            }

            pres->presentation_id = prsl->preselection_tag;

            /* reallocate label string if needed */
            if (prsl->label && (!pres->label || strlen(pres->label) < strlen(prsl->label)))
            {
                pres->label = realloc(pres->label, strlen(prsl->label) + 1);
                *presentations_changed = 1;
            }

            /* copy label from preselection */
            if (prsl->label && strcmp(pres->label, prsl->label))
            {
                memcpy(pres->label, prsl->label, strlen(prsl->label) + 1);
                *presentations_changed = 1;
            }

            /* reallocate language string if needed */
            if (prsl->extended_language && (!pres->language || strlen(pres->language) < strlen(prsl->extended_language)))
            {
                pres->language = realloc(pres->language, strlen(prsl->extended_language) + 1);
                *presentations_changed = 1;
            }

            /* copy language from preselection */
            if (prsl->extended_language && strcmp(pres->language, prsl->extended_language))
            {
                memcpy(pres->language, prsl->extended_language, strlen(prsl->extended_language) + 1);
                *presentations_changed = 1;
            }
        }
    }
